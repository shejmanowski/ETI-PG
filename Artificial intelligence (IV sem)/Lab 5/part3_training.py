...
def train(model, train_data, valid_data=None, exp_dir='exp_00'):
    ...
    # my callbacks
    es_cbk = tf.keras.callbacks.EarlyStopping(patience=10)
    ckp_cbk = tf.keras.callbacks.ModelCheckpoint(f'{exp_dir}/model_best_weights', save_best_only=True, save_weights_only=True)
    tb_cbk = tf.keras.callbacks.TensorBoard(log_dir=exp_dir)

    # training
    if valid_data is None:
        history = model.fit(x=x, y=y, batch_size=64, validation_split=0.2,
                            epochs=100, verbose=1, callbacks=[es_cbk,
                                                                ckp_cbk, tb_cbk])
    else:
...
def get_random_hp(constants):
    ...
    return FFN_Hyperparams(constants[0], constants[1], hidden_dims=[random.choice(hidden_dims_values_range),
                                                                    random.choice(hidden_dims_values_range),
                                                                    random.choice(hidden_dims_values_range)],
                            activation_fcn=random.choice(activation_fcn_choices), learning_rate=10**random.choice(lr_exponent_range))
