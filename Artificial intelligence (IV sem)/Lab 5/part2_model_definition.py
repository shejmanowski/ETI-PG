...
def build_model(hp: FFN_Hyperparams):
    ...
    # TODO
    model.add(layers.Dense(hp.hidden_dims[0], activation=hp.activation_fcn, input_shape=[hp.num_inputs], name='ukryta1'))
    model.add(layers.Dense(hp.hidden_dims[1], activation=hp.activation_fcn, name='ukryta2'))
    model.add(layers.Dense(hp.hidden_dims[2], activation=hp.activation_fcn, name='ukryta3'))
